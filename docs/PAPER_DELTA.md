...
| D22 | `web/flutter_bootstrap.js` | **The offline Capstone Manual (N14) still failed on a refresh, even with the caching service worker restored by D21.** `flutter_service_worker.js` caches the app shell during install under the name in its CORE list — `index.html` — writing the entry `<origin>/index.html`. A browser reloading the site root asks for `<origin>/`, which the worker maps to the key `/`. Those are two different cache entries, and `/` is written only opportunistically, by `onlineFirst` after a *successful online* navigation through an already-active worker. On a first visit the worker installs after that navigation completes, so `/` is never written: the student loads Appstone, loses connection, refreshes, and gets the browser's own "You're offline" page — while `main.dart.js`, `canvaskit/` and every asset sit correctly in the cache. Only a second online load repaired it, which is not something a student can be expected to know | ✅ Once the worker controls the page, the bootstrap re-requests the root once. The request passes through the worker's own fetch handler, so the response is stored under `/` — the key an offline refresh matches — without this file hardcoding the worker's cache name. Fires on `controllerchange` (first visit) and on `flutter-first-frame` (later visits, off the critical path). It doubles as a refresh of the offline copy of the shell |
...
| C24 | Each AI feature had exactly one provider. If NaraRouter was still rate-limited after the proxy's three backoff retries, the paper checker, AI workflow and defense practice all returned "The AI service is busy right now" — even though Groq was sitting idle with a separate 30/min allowance, and vice versa for the title generator | The proxy now fails over between the two providers. A 429 that survived the backoff (one ~3s retry where a stand-in exists, the full ~21s schedule where none does), a 5xx, or an unreachable provider sends the same request to the other one, with the model name swapped for one that provider serves (`agnes-2.0-flash` ↔ `openai/gpt-oss-20b`; NaraRouter's model was `mistral-large` until it went down, see D23). The student is never told which one answered. Both allowances have to be exhausted before anyone sees "busy", so the app's ceiling is now NaraRouter's 10/min **plus** Groq's 30/min rather than whichever one the feature happened to be pinned to. Speech-to-text is the exception: Whisper is audio and NaraRouter routes chat only, so a transcription has nowhere to fail over to and behaves exactly as before | ✅ |
...
| C29 | Signing in with no connection reported **"That password does not match."** under the password field. The sign-in catch treated every `FirebaseAuthException` as a credential failure, so `network-request-failed` - which means the password was never checked at all - accused the student of typing it wrong. Firebase retried internally first, so the wrong answer took about ten seconds to arrive | The attempt is refused up front while the browser reports no connection, with a banner saying they are offline, that the password is checked on the server, and to reconnect. Failures still classify themselves rather than trusting that check: only `invalid-credential` / `wrong-password` / `user-not-found` set the password field error now, connection failures set the offline banner, and every other code goes through `friendlyErrorMessage` instead of being called a wrong password. The login screen also carries the N15 offline strip, so it says so before the attempt | ✅ |
| C30 | The Manuscript Checker discarded the previous check's remarks the instant a new file was picked - before the new check had started, and whether or not it ever ran. A failed run did the same: the error replaced the results pane, so hitting the daily AI allowance on a second draft cost the student the feedback they were working from | A finished review now survives until a new one replaces it. Picking a file clears only a stale error; the run builds its result in locals and commits in one step, so a failure leaves the previous remarks standing. Two notes keep it unambiguous: which file the visible remarks came from once a different one is selected, and a danger-tinted banner carrying the failure above the retained results. Nothing stale shows mid-run - the pane renders its running state throughout | ✅ |
| C31 | C30 kept a finished check on screen only for as long as the tab stayed open. The result lives in `PaperCheckController`, an in-memory singleton, so a browser refresh, a PWA relaunch or any restart wiped it - the student came back to "No check yet" even though the check was already saved in `paper_checks` and listed on the Check History screen | The Manuscript Checker now reads its last saved check back on open, so the remarks persist across restarts and change only when a new manuscript is checked. It is a re-presentation of an existing record, not a new store: `PaperCheckHistoryService.fetchLatest` returns the newest check and the controller commits it, once per app run, and never over a check in flight or one finished this session. A note names the file and when it was graded so a restored result is never mistaken for a fresh run; a skeleton covers the fetch so the empty state never flashes first. A restored check shows its formatting tally (`X of Y rules met`) but not the individual rules, which were measured from a `.docx` the app no longer holds. The controller also remembers whose result it holds and wipes it when a different student signs in on the same device - previously one student's remarks could greet the next | ✅ |
...
| D23 | `functions/index.js` (`FALLBACK_MODEL.nararouter`), `defense_ai_service.dart`, `paper_checker_service.dart`, `workflow_service.dart` | Mistral Large, the model behind NaraRouter for the paper checker, AI workflow and defense practice, went down | ✅ Model constant switched to `agnes-2.0-flash` on NaraRouter in all four sites (the three services plus the proxy's own fallback target), until Mistral Large is back |
...
- **AI providers** — NaraRouter, and Groq (`openai/gpt-oss-20b`, `whisper-large-v3`) via the us-central1 relay. Still the same two providers, still reached only through the proxy. NaraRouter's model was Mistral Large until it went down; it now serves `agnes-2.0-flash` (see D23). **Routing changed** — see C24: each feature still has a home provider (paper checker / workflow / defense on NaraRouter, title generation and speech on Groq), but a chat feature whose provider is rate-limited or down is now answered by the other one instead of failing. Speech has no stand-in.
